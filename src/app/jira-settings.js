define(['ko', 'jquery', "objecter", 'promise-monad', "swagger-client","rooter", "ko.extensions"], function (ko, $, O, PM, swagger,rooter) {
  ...
  var restServer = new (function RestServer(jSessionID) {
    ...
    function allMessages(exc) {
      var messages = [(exc || {}).ExceptionMessage];
      var innerException = ((exc || {}).InnerException)
      if (!innerException) return messages.filter(function (m) { return m; });
      return messages.concat(allMessages(innerException));
    }
    function swaggerErrorParser(e) {
      //e.obj.InnerException.ExceptionMessage
      var messages = allMessages(e.obj);
    ...
    this.postIssueTransition = function postIssueTransition(ticket, transition, comment) {
      return swPostTrans(ko.unwrap(ticket), ko.unwrap(transition), ko.unwrap(comment), ko.unwrap(jSessionID));
    };
    ...
    var exports = new swagger({
      ...
      success: function () {
        ...
        swPostTrans = function (ticket, transition, comment, sessionID) {
          var d = $.Deferred();
          exports.Jira[O.name(exports.Jira, "Jira_TransitionTicket")]({ ticket: ticket, transition: transition, comment: comment, sessionID: sessionID }, d.resolve.bind(d), d.reject.bind(d));
          return d;
        };
